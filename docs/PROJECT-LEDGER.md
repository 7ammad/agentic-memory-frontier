...
- Follow-up: Push the Phase-3 slice branch (`feat/cem1-phase3-action-value-retrieval`, stacked on PR#4) as its own small PR to staging; run the bounded Greptile loop to 5/5; stop before merge. Then Phase 4 (MMA + 10-baseline exam with the >=5pp lexical margin and leakage guard — the single-shot held-out run uses the weights pinned above), Phase 5 (hardening, incl. the two latent nits below), and the §12 correction-capture live-hooks track.
